...
class Solution:
    def goodNodes(self, root: TreeNode) -> int:
        return self.goodNodesRecursive(root, root.val, 0)
    
    def goodNodesRecursive(self, current: TreeNode, largestSoFar: int, count: int):
        ...
        if current.val >= largestSoFar: # If the current node is a Good Node.
            return ( self.goodNodesRecursive(current.left, largestSoFar, count) + self.goodNodesRecursive(current.right, largestSoFar, count) + 1)
        else: # Current node is not a Good Node.
            return ( self.goodNodesRecursive(current.left, largestSoFar, count) + self.goodNodesRecursive(current.right, largestSoFar, count))
