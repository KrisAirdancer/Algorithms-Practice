class Solution:
    ...
    def quicksort(self, nums, left, right):
        ...
        pivot = randint(left, right) # Calculate the pivot.
...
#   > Random Quicksort reduces the risk of O(N^2) performance in some cases b/c it prevents inversely sorted (or almost inversely sorted) lists from forcing the selection of the worst pivot on each iteration by randomly selecting a pivot.
...
# OUTLINE - Quicksort
# Select a pivot as a random element in the array
# Move the pivot element to the right end of the array
